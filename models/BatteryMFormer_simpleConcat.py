'''
This is the BatteryMFormer
1. w/o aging-condition-aware attention;
2. and change the additive condition mechanism to simple concatenation
'''
import math
...
class Model(nn.Module):
    ...
    def __init__(self, configs, init_value_memory=None):
        ...
        self.learnable_queries = nn.Parameter(torch.rand(self.num_query-1, self.d_model))
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, soh_trajectory=None, trajectory_mask=None,
        ...
        decoder_input = torch.cat([shared_queries, self.aging_condition_query_linear(condition_embed)], dim=1)
        decoder_output = self.decoder(decoder_input, source_x, cross_mask=cross_mask)
