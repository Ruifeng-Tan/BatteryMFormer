...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.intra_embed = nn.Linear(self.charge_discharge_length * 4, self.d_model)
        self.intra_MLP = nn.ModuleList([
            MLPBlock(self.d_model, self.d_ff, self.d_model, self.dropout) 
            for _ in range(self.e_layers)
        ])
        self.feature_embed = nn.Linear(3, self.d_model)
        ...
        self.aging_condition_embed_mlp = nn.Sequential(nn.Linear(self.d_llm, self.d_model), nn.GELU(), 
        ...
        self.num_visible_cycles = self.early_cycle_threshold - self.seq_len + 1 + 1
    ...
    def forecast_soh_from_curves(self, cycle_curve_data, curve_attn_mask, condition_embed, manual_features):
        """
        ...
        Args:
            ...
            condition_embed: [B, 1, d_model]
            manual_features: [B, early_cycles, num_features]
        Returns:
        ...
        cycle_embeddings = self.feature_embed(manual_features) + cycle_embeddings
        cycle_embeddings = torch.cat([condition_embed, cycle_embeddings], dim=1) # [B, early_cycles+1, d_model]
        cycle_embeddings = self.pe(cycle_embeddings) + cycle_embeddings
        
        # Prepare attention mask for encoder
        one_ones_mask = torch.ones_like(curve_attn_mask[:,:1]) # [B, 1]
        enc_self_mask = self._get_attention_mask(torch.cat([one_ones_mask, curve_attn_mask], dim=1))
    ...
    def forward(self, cycle_curve_data=None, curve_attn_mask=None,
        ...
        if self.configs.input_mode != 'current_voltage':
            ...
            output = self.forecast_soh_from_curves(cycle_curve_data, curve_attn_mask, condition_embed, manual_features)
