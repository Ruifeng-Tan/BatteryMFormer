...
(Optional) aging-condition embeddings can be regenerated by running `generate_aging_condition_embeddings.ipynb`. This step can be skipped for standard experiments because the pre-computed Qwen3 embeddings are already included at `data_provider/prompt_embeddings/Qwen3_total.pkl`.
