"""
MICH Dataset SOH PCHIP Interpolation Smoothing Script

Features:
1. Detect anomalies in SOH curves (test-induced increases/decreases)
2. Special handling for Formation test cycles (Cycle 344, 444, 493, 534)
3. Fix anomalies using PCHIP interpolation
4. Record anomaly cycle information
...
# Parameter Configuration (Based on MICH analysis)
# FIXED: Previous values were too strict (THRESHOLD_INCREASE=0.01, RECOVERY_TOLERANCE=0.0001)
# which caused the avalanche effect
THRESHOLD_INCREASE = 1.0      # 1.0% increase threshold (more reasonable)
THRESHOLD_DECREASE = -1.7     # -1.7% decrease threshold (1st percentile: -4.78%, conservative)
THRESHOLD_EXTREME = 10.0      # 10% extreme anomaly threshold (for Cycle 444 pattern)
STABILITY_THRESHOLD = 0.8     # Stability detection: consecutive changes < 0.8%
...
RECOVERY_TOLERANCE = 0.02     # SOH needs to recover within 2% of pre-anomaly level (FIXED from 0.0001)
...
def detect_anomalies(soh: np.ndarray, cycle_numbers: np.ndarray) -> List[Dict]:
    """Detect anomalies in SOH curve including special Formation test cycles"""
    anomalies = []
    relative_changes = calculate_relative_changes(soh)

    i = 1
    while i < len(soh):
        current_cycle = cycle_numbers[i]
        ...
        # Check if current cycle is a special Formation test cycle
        is_special_cycle = current_cycle in SPECIAL_CYCLES

        # Detect extreme anomalies (especially for special cycles)
        if abs(relative_changes[i]) > THRESHOLD_EXTREME:
            start_idx = i - 1
            ...
            anomaly_type = 'extreme_increase' if max_change > 0 else 'extreme_decrease'
            if is_special_cycle:
                anomaly_type += '_formation'
            ...
            anomalies.append({
                ...
                'is_formation_cycle': is_special_cycle
            })

            i = end_idx + 1
            continue

        # Detect large increases (normal threshold)
        if relative_changes[i] > THRESHOLD_INCREASE:
            ...
            anomaly_type = 'increase_formation' if is_special_cycle else 'increase'
        ...
        # Detect large decreases (normal threshold)
        if relative_changes[i] < THRESHOLD_DECREASE:
            ...
            anomaly_type = 'decrease_formation' if is_special_cycle else 'decrease'
...
def process_battery(pkl_file: Path) -> Dict:
    ...
    for idx, anomaly in enumerate(anomalies):
        formation_marker = " [FORMATION]" if anomaly.get('is_formation_cycle', False) else ""
        print(f"    [{idx+1}] {anomaly['type']}{formation_marker}: cycle {anomaly['start_cycle']}-{anomaly['end_cycle']} "
...
def main():
    """Main function"""
    print("=" * 80)
    print("MICH Dataset SOH PCHIP Interpolation Smoothing")
    print("=" * 80)
    ...
    print(f"  Extreme threshold: {THRESHOLD_EXTREME}%")
    print(f"  Special cycles: {SPECIAL_CYCLES} (Formation test cycles)")
