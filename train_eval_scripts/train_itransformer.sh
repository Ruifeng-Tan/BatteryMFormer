...
# iTransformer Training Script for MemoryNet
# Inverted Transformer for SOH trajectory forecasting
# Paper: https://arxiv.org/abs/2310.06625
# Usage:
#   Single GPU: bash train_eval_scripts/train_itransformer.sh [current_voltage|soh_to_soh|both]
#   Multi GPU:  bash train_eval_scripts/train_itransformer.sh [current_voltage|soh_to_soh|both] [num_gpus]
...
if [ "$MODE" = "current_voltage" ]; then
    ...
    echo "Usage: bash train_eval_scripts/train_itransformer.sh [current_voltage|soh_to_soh|both] [num_gpus]"
    exit 1
fi
